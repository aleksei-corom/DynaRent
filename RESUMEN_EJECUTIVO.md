...
> **Fecha:** 2026-08-14 · **Estado general:** listo para producción — release v1.0.9 publicada por CI, firmada para el auto-update, validada de punta a punta en Windows limpio y verificada en el equipo de operaciones (contrato en 2 hojas, campo Gasolina, errores de BD visibles).
...
| **Versión estable** | **v1.0.9** — la única release que se distribuye (con auto-update activo) |
...
| **Auto-actualización** | ✅ Activa desde la **v1.0.3** — la app chequea `latest.json` al arrancar y ofrece instalar (firma minisign verificada) | ✅ Secret `TAURI_SIGNING_PRIVATE_KEY` configurado; v1.0.4–v1.0.9 publicadas y firmadas |

## 2. Releases en GitHub

| Release | Estado | Para quién |
|---|---|---|
| **v1.0.9** | ✅ **Latest / estable** — construida íntegramente por CI (GitHub Actions) | **Única descarga recomendada** |
| v1.0.3 | ✅ Estable anterior (sigue funcionando) | Actualizar a v1.0.9 (auto-update) |
...
**Assets de la v1.0.9:** [`DinamoRent_1.0.9_x64-setup.exe`](https://github.com/CORJAR-Computers/dinamo_rent_tr/releases/download/v1.0.9/DinamoRent_1.0.9_x64-setup.exe) (NSIS, ~21 MB, recomendado) y [`DinamoRent_1.0.9_x64_en-US.msi`](https://github.com/CORJAR-Computers/dinamo_rent_tr/releases/download/v1.0.9/DinamoRent_1.0.9_x64_en-US.msi) (~33 MB, despliegue GPO; sha256 publicados en la release). La release incluye además los `.sig` y el `latest.json` para el auto-update. Enlaces y credenciales iniciales en [`INSTALACION_OPERACIONES.md`](INSTALACION_OPERACIONES.md).
...
**Qué añaden las v1.0.4 → v1.0.9** (14-08, diagnóstico y documento):
...
| **`verificar-despliegue.ps1`** | Post-instalación en el equipo del cliente: exe v1.0.9, arranque vivo 10 s, `config.ini` + BD del primer arranque — veredicto OK/FALLOS | `powershell -File scripts/verificar-despliegue.ps1` |
