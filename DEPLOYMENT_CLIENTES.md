# Plan de despliegue en equipos de clientes — Dinamo Rent v1.0.3

> Procedimiento operativo para dejar los equipos de los clientes en la **v1.0.3** (primera
> versión con **auto-actualización**): instalación silenciosa, verificación post-instalación
> y rollback. **Este es el último despliegue manual por equipo**: desde la v1.0.3 la app
> detecta y ofrece las versiones nuevas al arrancar. Complementa a
> `INSTALACION_OPERACIONES.md` (enlaces de descarga y credenciales iniciales).

---

## 0. Reglas de oro

1. **Siempre la v1.0.3 (o superior)** — la v1.0.0 está descontinuada (falla en
   instalaciones nuevas) y la v1.0.2 no tiene updater (se actualiza una vez a mano).
...
> v1.0.3 encima y verificar. Solo hay que confirmar el backup antes.
...
> Este procedimiento manual se ejecuta **una última vez por equipo** (la transición a la
> v1.0.3). Las versiones siguientes llegan solas por el auto-update de la app: no hace falta
> volver a tocar los equipos ni el share.

```powershell
# NSIS — silenciosa total (sin atajos, sin ejecutar al final)
& "D:\deploy\DinamoRent_1.0.3_x64-setup.exe" /S
# Esperar a que termine (NSIS /S es síncrono al esperar al proceso)
# Start-Process -Wait -FilePath "D:\deploy\DinamoRent_1.0.3_x64-setup.exe" -ArgumentList "/S"
```

```powershell
# MSI — para GPO / Intune / SCCM
msiexec /i "D:\deploy\DinamoRent_1.0.3_x64_en-US.msi" /qn /norestart
```
...
psexec \\PC-CLIENTE-01 -s -d "D:\deploy\DinamoRent_1.0.3_x64-setup.exe" /S
```
...
|---|---|---|
| 1 | Exe instalado (`%LOCALAPPDATA%\DinamoRent\dinamo-rent.exe`) | existe, versión **1.0.3** |
...
| Síntoma | Acción |
|---|---|
| Exe no aparece / versión no es 1.0.3 | Reinstalar (¿el instalador correcto? ¿se descargó una versión anterior?) |
| `config.ini` pero NO la BD | No borrar nada: reinstalar la v1.0.3 (el arranque crea la BD). Si persiste, revisar exclusión de Defender sobre la carpeta |
...
[ ] Instalador v1.0.3 descargado (verificar hash/tamaño ~21 MB)
