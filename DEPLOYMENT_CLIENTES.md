# Plan de despliegue en equipos de clientes — Dinamo Rent v1.0.1

> Procedimiento operativo para instalar la **v1.0.1** (única versión estable) en los
...
1. **Siempre la v1.0.1** — la v1.0.0 está descontinuada (falla en instalaciones nuevas).
...
> v1.0.1 encima y verificar. Solo hay que confirmar el backup antes.
...
& "D:\deploy\DinamoRent_1.0.1_x64-setup.exe" /S
# Esperar a que termine (NSIS /S es síncrono al esperar al proceso)
# Start-Process -Wait -FilePath "D:\deploy\DinamoRent_1.0.1_x64-setup.exe" -ArgumentList "/S"
```

```powershell
# MSI — para GPO / Intune / SCCM
msiexec /i "D:\deploy\DinamoRent_1.0.1_x64_en-US.msi" /qn /norestart
```
...
psexec \\PC-CLIENTE-01 -s -d "D:\deploy\DinamoRent_1.0.1_x64-setup.exe" /S
```
...
| Síntoma | Acción |
|---|---|
| Exe no aparece / versión no es 1.0.1 | Reinstalar (¿el instalador correcto? ¿se descargó la v1.0.0?) |
| `config.ini` pero NO la BD | No borrar nada: reinstalar la v1.0.1 (el arranque crea la BD). Si persiste, revisar exclusión de Defender sobre la carpeta |
...
> En la práctica, con la v1.0.1 **no debería hacer falta un rollback de BD**: el arranque
...
[ ] Instalador v1.0.1 descargado (verificar hash/tamaño ~21 MB)
