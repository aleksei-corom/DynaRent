...
> **Última versión estable: [v1.0.3](https://github.com/CORJAR-Computers/dinamo_rent_tr/releases/tag/v1.0.3)** — añade el **auto-update**: la app detecta y ofrece instalar las versiones nuevas al arrancar (firma verificada), sobre las features de la v1.0.2 (IVA por renta, auto-cálculo de días/horas al cerrar, cambio de vehículo, combos con búsqueda). Construida y validada por CI en Windows limpio.

### 1. Descargar el instalador

Ve a la página de [releases de GitHub](https://github.com/CORJAR-Computers/dinamo_rent_tr/releases) y descarga de la **v1.0.3**:

| Instalador | Cuándo usarlo |
|---|---|
| `DinamoRent_1.0.3_x64-setup.exe` (NSIS, ~21 MB) | **Recomendado** — instalación asistida con atajo de escritorio |
| `DinamoRent_1.0.3_x64_en-US.msi` (MSI, ~31 MB) | Despliegue empresarial / GPO (instalación silenciosa con `msiexec`) |

> ⚠️ **No uses la v1.0.0** (descontinuada): en equipos nuevos sin BD previa se colgaba antes de llegar al Login. Si ya la tienes instalada **con datos**, no necesitas desinstalar — la v1.0.3 abre tu BD actual tal cual.
...
Solo instala la v1.0.3 encima (o desinstala y reinstala conservando `%APPDATA%\com.corjar.dinamorent\`): el arranque es idempotente y aplica únicamente las migraciones pendientes. **No se pierde ningún dato.**
...
| **[INSTALACION_OPERACIONES.md](INSTALACION_OPERACIONES.md)** | 🚀 Guía de instalación de la v1.0.3 para operaciones: enlaces a los assets, credenciales iniciales y verificación |
...
**Versión estable**: 1.0.3 (construida y validada por CI — ver [releases](https://github.com/CORJAR-Computers/dinamo_rent_tr/releases)). La versión legacy de la migración Tauri V2 era 4.0.0-beta; desde la **v1.0.0** el versionado sigue el semver del proyecto (1.0.x).
