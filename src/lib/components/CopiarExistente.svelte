...
<div class="mb-4 rounded-xl border border-primary/20 bg-primary/5 overflow-hidden">
	...
	>
		...
		<kbd
			class="hidden sm:inline-flex items-center rounded border border-primary/25 bg-primary/10 px-1.5 py-0.5 text-[10px] font-mono text-primary/80 leading-none"
			>Ctrl+Shift+C</kbd
		>
		<svg
	...
	{#if panelCopiar}
		<div class="px-3 pb-3 pt-2.5 border-t border-primary/10">
			...
			{#if resultadosVisibles.length > 0}
				<ul
					class="mt-2 max-h-52 overflow-y-auto rounded-lg border border-border bg-surface divide-y divide-border/60"
				>
					{#each resultadosVisibles as r (r.id)}
						<li>
							<button
								...
								title={r.bloqueado
									? (r.razonBloqueo ?? 'No se pueden copiar los datos.')
									: `Copiar datos de ${r.titulo}`}
