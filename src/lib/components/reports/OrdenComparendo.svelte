...
<div class="print-area bg-white text-slate-900">
	...
	<div class="border-b-2 border-slate-800 pb-4 mb-5 flex items-start justify-between gap-4">
		<div class="flex items-center gap-3">
			<div
				class="w-16 h-16 rounded-lg bg-red-900 text-white flex items-center justify-center shrink-0 p-1.5"
			>
				<img
					src={empresa.logoSrc}
		...
		<div class="text-right">
			...
			<p class="text-sm font-semibold mt-1">
				Ref. <span class="tabular-nums">{String(comparendo.id).padStart(4, '0')}</span>
			</p>
	...
	<div class="flex items-center gap-2 mb-5">
		...
		<span
			...
			{comparendo.estado === 'Pendiente'
				? 'border-orange-700 text-orange-700'
				: comparendo.estado === 'Pagado'
	...
	<div class="grid grid-cols-2 gap-4 mb-5">
		<div class="rounded-lg border border-slate-300 p-3">
			...
			<p class="text-xs text-slate-600">
				Placa: <span class="font-semibold text-slate-800">{comparendo.placa}</span>
			</p>
		</div>
		<div class="rounded-lg border border-slate-300 p-3">
			<p class="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">
				Responsable (Cliente / Renta)
			</p>
			{#if comparendo.responsable}
				<p class="font-bold">{comparendo.responsable.nombreCliente || 'Cliente sin nombre'}</p>
				<p class="text-xs text-slate-600">
					Renta {formatContrato(
						comparendo.responsable.anioContrato,
						comparendo.responsable.noContrato
					)}
					<span class="text-slate-400"
						>· {formatDate(comparendo.responsable.fechaRecogida)} → {formatDate(
							comparendo.responsable.fechaRetorno
						)}</span
					>
				</p>
			...
			{:else}
				...
				<p class="text-[10px] text-slate-500 mt-0.5">
					El vehículo no estaba rentado el día de la infracción.
				</p>
			{/if}
		</div>
	</div>
	...
	<div class="rounded-lg border border-slate-300 p-3 mb-5">
		...
		<div class="space-y-2 text-sm">
			<div class="grid grid-cols-2">
				<p>
					<span class="text-slate-500 inline-block w-20">Fecha:</span>
					<span class="font-semibold">{formatDate(comparendo.fechaInfraccion)}</span>
				</p>
				<p>
					<span class="text-slate-500 inline-block w-20">Hora:</span>
					<span class="font-semibold">{hora(comparendo.horaInfraccion)}</span>
				</p>
			</div>
			<div
				class="pt-2 border-t border-slate-100 flex justify-between items-center bg-slate-50 p-2 rounded"
			>
	...
	<div class="text-[10px] text-slate-600 space-y-0.5 mb-8">
		<p>
			...
			renta estipulado en el contrato.
		</p>
		<p>
			• La presente notificación informa del registro de una infracción de tránsito asociada al
			vehículo y periodo de renta correspondientes.
		</p>
	...
	<div class="grid grid-cols-2 gap-8 mt-10">
		<div class="text-center">
			<div class="border-t border-slate-400 pt-2 text-xs font-semibold">
