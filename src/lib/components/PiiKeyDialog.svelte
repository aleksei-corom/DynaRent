<script lang="ts">
	...
	async function probar() {
		...
		try {
			...
			estado = r;
			probandoInfo = r.clientesDescifrados > 0
	...
	async function eliminar() {
		if (!confirm('¿Eliminar la clave PII configurada? Los datos Fernet legacy volverán a ocultarse.')) return;
		guardando = true;
		try {
...
>
	{#snippet children()}
		...
		{#if estado}
			<div
				class="mb-4 rounded-xl border px-4 py-3 flex items-start gap-3 {tieneClave ? 'border-exito/25 bg-exito/5' : 'border-alerta/25 bg-alerta/5'}"
			>
		...
		{:else}
			<div class="mb-4 rounded-xl border border-border bg-alt-row/60 px-4 py-3 text-xs text-text-secondary animate-pulse">
		...
		<!-- Entrada de clave -->
		<div class="mb-2">
			<label class="label" for="pii-key-input">Clave original de la app anterior (db_encryption_key)</label>
			<div class="relative">
				<input
					id="pii-key-input"
			...
			<p class="mt-1 text-xs text-text-secondary/70">
				Se guarda en <code class="font-mono bg-alt-row px-1 py-0.5 rounded">config.ini</code> [security] y se aplica sin reiniciar la app.
			</p>
		</div>

		{#if probandoError}
			<div class="mb-3 rounded-lg bg-peligro/10 border border-peligro/30 px-3 py-2 text-sm text-peligro" role="alert">{probandoError}</div>
		{/if}
		{#if probandoInfo}
			<div class="mb-3 rounded-lg bg-exito/10 border border-exito/30 px-3 py-2 text-sm text-exito">{probandoInfo}</div>
		{/if}

		<div class="flex flex-wrap items-center gap-2 mt-2">
			<button class="btn-outline !px-3 !py-1.5 text-xs" onclick={probar} disabled={probando || guardando}>
				{#if probando}
			...
			{#if tieneClave}
				<button class="btn-ghost !px-3 !py-1.5 text-xs text-peligro hover:text-peligro" onclick={eliminar} disabled={guardando}>
					Eliminar clave
				</button>
			{/if}
		</div>
