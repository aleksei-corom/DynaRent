...
<FormField label={label ?? ''} {hint} {required} {dense} class={klass}>
	<div bind:this={rootRef} class="relative">
		<div class="relative">
			<input
				bind:this={inputRef}
				class="input pr-8 {abierto ? 'rounded-b-none border-b-transparent' : ''} {disabled
			...
			/>
			<svg
				class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-text-secondary/60 transition-transform {abierto
					? 'rotate-180'
					: ''}"
