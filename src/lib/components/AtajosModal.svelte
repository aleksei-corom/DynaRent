<script module lang="ts">
	...
	export const ATAJOS_APP: { teclas: string[]; descripcion: string }[] = [
		...
		{
			teclas: ['Ctrl+K'],
			descripcion: 'Abrir la búsqueda general de páginas (paleta de comandos)'
		},
		{
			teclas: ['Ctrl+Shift+C'],
			descripcion: 'Copiar datos de un cliente o vehículo existente (en los modales de crear)'
		},
...
>
	{#snippet children()}
				...
				<li class="flex items-center justify-between gap-4 py-3">
					...
					<span class="flex items-center gap-1.5 shrink-0">
						{#each atajo.teclas as tecla, i}
							...
							<kbd
								class="inline-flex items-center rounded-md border border-border bg-alt-row/60 px-2 py-1 text-[11px] font-mono text-text-primary leading-none"
								>{tecla}</kbd
							>
						{/each}
					</span>
		...
		<p
			class="mt-4 pt-3 border-t border-border text-xs text-text-secondary flex items-start gap-1.5"
		>
