...
- **Assets de la v1.0.13 (al publicar):** los 2 instaladores (`DynaRent_1.0.13_x64-setup.exe` NSIS ~21 MB y `DynaRent_1.0.13_x64_en-US.msi` ~33 MB), sus firmas del updater (`*.exe.sig` / `*.msi.sig`) y `latest.json`. El sha256 de los instaladores se calcula al publicar y se pega en esta sección.
